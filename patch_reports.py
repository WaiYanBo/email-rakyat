...
new_logic = '''
  const hasFullAccess = permissions.view_staff;
'''

content = re.sub(r'''
  const hasFullAccess = \['Chairman', 'CEO', 'COO', 'CFO', 'General Manager', 'IT Admin'\].includes\(profile\?\.role\);''', new_logic, content, flags=re.VERBOSE | re.DOTALL)

open('src/components/ReportsView.tsx', 'w', encoding='utf-8').write(content)
print("ReportsView patched")
