...
export default function SettingsView() {
  ...
  const handleUpdateProfile = async (e: React.FormEvent) => {
    ...
    if (!cleanName) {
      setProfileMessage({ type: 'error', text: t('settings', 'nameEmptyError', lang) });
      setIsSavingProfile(false);
      return;
    }
    if (!isValidName(cleanName)) {
      setProfileMessage({ type: 'error', text: t('settings', 'nameInvalidError', lang) });
      setIsSavingProfile(false);
      return;
    }
  ...
  const handleUpdatePassword = async (e: React.FormEvent) => {
    ...
    if (!oldPassword) {
      setPasswordMessage({ type: 'error', text: t('settings', 'passwordRequired', lang) });
      return;
    }

    const strength = isStrongPassword(newPassword, lang);
    if (!strength.valid) {
    ...
    if (newPassword !== confirmPassword) {
      setPasswordMessage({ type: 'error', text: t('settings', 'passwordsDoNotMatch', lang) });
      return;
    }

    if (oldPassword === newPassword) {
      setPasswordMessage({ type: 'error', text: t('settings', 'passwordSameAsCurrent', lang) });
      return;
    }
    ...
    try {
      ...
      if (verifyError) {
        setPasswordMessage({ type: 'error', text: t('settings', 'passwordIncorrect', lang) });
      ...
      setPasswordMessage({ type: 'success', text: t('settings', 'passwordUpdateSuccess', lang) });
      setOldPassword('');
      setNewPassword('');
      setConfirmPassword('');
    } catch (_err) {
      setPasswordMessage({ type: 'error', text: t('settings', 'passwordUpdateError', lang) });
    } finally {
      setIsUpdatingPassword(false);
    }
  };
  ...
  const isITAdmin = roleName?.toLowerCase() === 'it' || roleName?.toLowerCase() === 'it admin' || profile?.department?.toLowerCase() === 'it';
    ...
    <div className="space-y-6">
      ...
      {(isITAdmin || permissions?.manage_access_control) && (
        <div className="flex bg-slate-100/50 dark:bg-gray-900/40 p-1 rounded-xl border border-slate-200 dark:border-gray-800 w-full sm:w-fit mx-auto mt-4 md:mt-0">
          ...
          >
            {t('settings', 'accessControlAdmin', lang)}
          </button>
        </div>
      )}
          ...
          <div className="lg:col-span-1 space-y-6">
            <div className="bg-white dark:bg-gray-900/50 border border-slate-200 dark:border-gray-800 rounded-2xl overflow-hidden shadow-sm">
              <div className="h-32 bg-gradient-to-r from-indigo-900 to-indigo-950 flex items-end justify-center pb-4 relative">
                <div className="absolute top-4 right-4 bg-indigo-550/20 text-indigo-300 px-3 py-1 rounded-full border border-indigo-500/30 text-[10px] font-semibold uppercase tracking-wider shadow-sm">
                  {profile?.status === 'Active' ? t('settings', 'statusBadge', lang) : (profile?.status || 'Active')}
                </div>
            ...
            <div className="bg-white dark:bg-gray-900/50 border border-slate-200 dark:border-gray-800 rounded-2xl p-6 space-y-4 shadow-sm">
              ...
              <p className="text-xs text-slate-500 dark:text-zinc-400 font-medium">
                {t('settings', 'langPrefSubtitle', lang)}
              </p>
            ...
            <div className="bg-white dark:bg-gray-900/50 border border-slate-200 dark:border-gray-800 rounded-2xl shadow-sm overflow-hidden">
              ...
              <form onSubmit={handleUpdateProfile} className="p-6 space-y-4 bg-white dark:bg-black">
                {profileMessage && (
                  <div className={`p-4 rounded-xl border text-xs font-semibold ${
                    profileMessage.type === 'success'
                      ? 'bg-emerald-50 dark:bg-emerald-500/10 border-emerald-100 dark:border-emerald-500/30 text-emerald-800 dark:text-emerald-400'
                      : 'bg-rose-50 dark:bg-rose-500/10 border-rose-100 dark:border-rose-500/30 text-rose-700 dark:text-rose-400'
                  }`}>
                    {profileMessage.text}
                  </div>
                )}
                ...
                <div className="space-y-1">
                  <label htmlFor="settings-full-name" className="block text-xs font-semibold text-slate-455 dark:text-zinc-400 uppercase tracking-wider">{t('settings', 'fullName', lang)}</label>
                  <input
                ...
                <div className="flex justify-end pt-2">
                  <button
                    type="submit"
                    disabled={isSavingProfile}
                    className="px-6 py-3 rounded-xl text-xs md:text-sm font-semibold bg-indigo-600 hover:bg-indigo-700 text-white dark:bg-yellow-500 dark:text-black border-0 dark:hover:bg-yellow-400 border border-indigo-600 disabled:opacity-50 transition-all shadow-sm min-h-[48px]"
                  >
