...
const SortHeader = ({ label, sortKey, currentSort, onClick }: any) => (
  <th 
    className="px-3 py-2.5 font-bold cursor-pointer hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors select-none group whitespace-nowrap text-xs text-gray-700 dark:text-gray-300 border-b border-gray-300 dark:border-gray-700 sticky top-0 bg-gray-100 dark:bg-gray-800 z-10 shadow-sm" 
...
export default function ClientTable({ clients, canEdit, onAddClick, onEditClick }: { clients: any[], canEdit: boolean, onAddClick: () => void, onEditClick: (client: any) => void }) {
  ...
  return (
    <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl overflow-hidden shadow-sm flex flex-col h-[75vh]">
      ...
      <div className="p-4 border-b border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 flex-shrink-0">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-4">
          <h3 className="text-sm font-black text-gray-900 dark:text-white uppercase tracking-widest">Client Database</h3>
          {canEdit && (
            <button 
              onClick={onAddClick}
              className="text-xs font-bold uppercase tracking-wider bg-teal-600 hover:bg-teal-700 text-white dark:bg-yellow-500 dark:hover:bg-yellow-600 dark:text-black px-4 py-2 rounded-md transition-colors shadow-sm"
            >
              + Add Client
            </button>
          )}
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="relative flex-1">
            ...
            <input 
              ...
              className="w-full pl-9 pr-3 py-2 bg-gray-50 dark:bg-gray-950 border border-gray-200 dark:border-gray-700 rounded-md text-xs text-gray-900 dark:text-white focus:ring-1 focus:ring-teal-500 dark:focus:ring-yellow-500 transition-all"
            />
          </div>
          <select 
            value={dateFilter}
            onChange={(e) => setDateFilter(e.target.value)}
            className="bg-gray-50 dark:bg-gray-950 border border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-300 text-xs rounded-md py-2 px-3 focus:ring-1 focus:ring-teal-500 dark:focus:ring-yellow-500 cursor-pointer"
          >
        ...
        <table className="w-full text-left border-collapse whitespace-nowrap">
          ...
          <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
            {filteredClients.length > 0 ? filteredClients.map((client) => {
              ...
              return (
                <tr key={rowId} className="hover:bg-teal-50/50 dark:hover:bg-gray-800/50 transition-colors group text-xs">
                  <td className="px-3 py-2 font-mono text-gray-500 dark:text-gray-400">{client.DATE}</td>
                  <td className="px-3 py-2 font-bold text-gray-900 dark:text-white truncate max-w-[150px]">{client.NAME}</td>
                  <td className="px-3 py-2 text-gray-600 dark:text-gray-400">{client["PHONE NUMBER"]}</td>
                  <td className="px-3 py-2 text-gray-600 dark:text-gray-400">{client["IC NUMBER"]}</td>
                  <td className="px-3 py-2 text-gray-600 dark:text-gray-300">{client["CASE CATEGORY"]}</td>
                  <td className="px-3 py-2 font-mono text-emerald-600 dark:text-emerald-400">{client["TOTAL PAID (RM)"] || '0'}</td>
                  <td className="px-3 py-2 font-mono text-red-600 dark:text-red-400">{client["PENDING (RM)"] || '0'}</td>
                  <td className="px-3 py-2 font-mono text-blue-600 dark:text-blue-400">{client["PACKAGE (RM)"] || '0'}</td>
